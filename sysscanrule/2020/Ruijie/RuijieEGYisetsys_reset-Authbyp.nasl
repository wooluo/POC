...
{
 script_id(51799193);
 name = "Ruijie EG Yi gateway setsys_reset.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"High");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /setsys_reset.htm file authorization bypass, device initialization, unauthorized access and deletion of configuration files.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && "remove-file config.text" >< req[2] && 'window.location.href="setsys_reset.htm";' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && "remove-file config.text"><ssl_req && 'window.location.href="setsys_reset.htm";'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
