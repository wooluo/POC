...
{
 script_id(51799211);
 name = "Ruijie EG Yi gateway nat_port.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"High");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /nat_port.htm file authorization bypass,  It can be used for port mapping and device control to help further attacks.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && 'var str = "ip nat inside source static ";' >< req[2] && 'var str = "sh ip nat rule' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && 'var str = "ip nat inside source static ";'><ssl_req && 'var str = "sh ip nat rule'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
