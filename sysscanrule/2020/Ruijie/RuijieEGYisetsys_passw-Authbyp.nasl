...
{
 script_id(51799191);
 name = "Ruijie EG Yi gateway setsys_passw.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"High");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /setsys_passw.htm file authorization bypass, and the super administrator web telnet authentication password can be modified.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && '<input type="password" name="web_passW" id="web_passW"' >< req[2] && '<button class="npe_butt" onClick="savePassword1();" mark="update">' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && '<input type="password" name="web_passW" id="web_passW"'><ssl_req && '<button class="npe_butt" onClick="savePassword1();" mark="update">'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
