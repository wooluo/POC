...
{
 script_id(51799194);
 name = "Ruijie EG Yi gateway setsys_strong.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"Medium");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /setsys_strong.htm file authorization bypass,it can strictly view configurations,device initialization, unauthorized access and deletion of configuration files.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && "Open the strict application of routing, application routing function will be more superior!" >< req[2] && 'down_explain_h : "Configuration Template Download and Import"' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && "Open the strict application of routing, application routing function will be more superior!"><ssl_req && 'down_explain_h : "Configuration Template Download and Import"'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
