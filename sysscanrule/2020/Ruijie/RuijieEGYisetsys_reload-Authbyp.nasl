...
{
 script_id(51799192);
 name = "Ruijie EG Yi gateway setsys_reload.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"Medium");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /setsys_reload.htm file authorization bypass,it can reload system.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && 'window.location.href="setsys_reload.htm";' >< req[2] && 'tip_concent : "The restart process may take 1 minute. Please wait patiently. The page will be refreshed automatically after device restart."' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && 'window.location.href="setsys_reload.htm";'><ssl_req && 'tip_concent : "The restart process may take 1 minute. Please wait patiently. The page will be refreshed automatically after device restart."'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
