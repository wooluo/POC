...
{
 script_id(51799212);
 name = "Ruijie EG Yi gateway setsys_backup.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"Medium");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /setsys_backup.htm file authorization bypass,it can view or modify system configuration files.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && 'window.location.href="setsys_backup.htm";' >< req[2] && '<FORM enctype="multipart/form-data" action="setsys_backup.htm" method' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && 'window.location.href="setsys_backup.htm";'><ssl_req && '<FORM enctype="multipart/form-data" action="setsys_backup.htm" method'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
