...
{
 script_id(51799195);
 name = "Ruijie EG Yi gateway snmp_server.htm - Authorization bypass";
 script_name(name);
 script_category(ACT_ATTACK);
 script_family("CNLocal");
 script_dependencies("find_service.nasl", "http_version.nasl");
 script_set_attribute(attribute:"risk_factor", value:"High");
 script_set_attribute(attribute:"description", value:"Ruijie EG Yi gateway /snmp_server.htm file authorization bypass,it can view or modify the configuration of the SNMP Sam protocol.");
...
function check_vuln(port){
	...
	con = url + req[1] + req[2];
	if( "200 ">< req[0] && "var data = request('exec', 'show snmp user for-web');" >< req[2] && 'var data = request("exec", "show snmp location");' >< req[2] ){
...
function check_vuln_ssl(port){
	...
	con = url + ssl_req;
	if( "200 O"><ssl_req && "var data = request('exec', 'show snmp user for-web');"><ssl_req && 'var data = request("exec", "show snmp location");'><ssl_req){
		if (report_verbosity > 0) security_hole(port:port, extra:con);
			  else security_hole(port);
	}
}
