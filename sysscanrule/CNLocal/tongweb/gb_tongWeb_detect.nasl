...
if(description)
{
 ...
 script_name(english:"TongWeb Server Services detection");
 script_set_attribute(attribute:"description", value:"Detect the TongWeb Server is running");
 script_summary("Detect the TongWeb Server is running");
...
{
    req = http_send_recv3(method: "GET", item:"/", port:port);
	if("Server:" >!< req[1]) exit();
	if ("Server:" >< req[1] && "TongWeb Server" >< req[1] && "HTTP/1.1" >< req[0]){
