...
if(description)
{
  ...
  script_cvs_date("$Date: 2018/01/29 17:50:16 $");

  script_name(english:"SNMP Port Scanner");
...
ports.');
  ...
  script_copyright(english:"This script is Copyright (C) 2004-2018 WebRAY, Inc.");
...
#if ( get_kb_item("PortscannersSettings/run_only_if_needed")) #get_kb_item("Host/full_scan") )
...
foreach tcp_port (sort(tcp_list))
{
 ...
 tcp_port_list = tcp_port_list + tcp_port + ",";
}
...
foreach udp_port (sort(udp_list))
{
 ...
 udp_port_list = udp_port_list + udp_port + ",";
}
...
{
	report = strcat(
     '\nSNMP scanner was able to retrieve the open port list\n',
     'with the community name: ', san_str(str:snmp_comm), '\n',
     'It found ', n_tcp, ' open TCP ports:\n', tcp_port_list, '\n',
     'and ', n_udp, ' open UDP ports:\n', udp_port_list, '\n');
    security_note(port: snmp_port, extra: report, proto:"udp");
}
